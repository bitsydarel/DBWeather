 ...
 */
class ManageLocationsActivity : BaseActivity(), ManageLocationsView, WithDependencies {
    @Inject
    lateinit var presenter: ManageLocationsPresenter
    ...
    override fun onCreate(savedState: Bundle?) {
        ...
        locationsAdapter = if (savedState == null) LocationsAdapter(mutableListOf()) else LocationsAdapter(savedState.getList<LocationWeatherModel>(LOCATIONS).toMutableList())
    ...
    override fun onSaveInstanceState(outState: Bundle?) {
        ...
        outState?.putList(LOCATIONS, locationsAdapter.locations)
    }

    /******************* Manage Locations Custom Part *******************/
    override fun setupView() {
        binding.manageLocationsToolbar.setNavigationOnClickListener { presenter.goBack(this) }
    ...
    override fun goBackToConfiguration() = onBackPressed()

    override fun showMessage(message: String) = binding.manageLocationsLayout.snack(message)
    ...
    private inner class SwipeToRemove internal constructor(dragDirs: Int, swipeDirs: Int) : ItemTouchHelper.SimpleCallback(dragDirs, swipeDirs) {
        ...
        override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) = presenter.removeLocation(this@ManageLocationsActivity, locationsAdapter.remove(viewHolder.adapterPosition))
    }
}