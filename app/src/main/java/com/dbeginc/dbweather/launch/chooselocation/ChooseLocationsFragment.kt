...
package com.dbeginc.dbweather.launch.chooselocation
...
class ChooseLocationsFragment : BaseFragment(), MVMPVView, ChooseLocationBridge, SearchView.OnQueryTextListener, WithSearchableData {
    ...
    private val locationAdapter by lazy { ChooseLocationAdapter(actionBridge = this) }
    override val stateObserver: Observer<RequestState> = Observer { onStateChanged(state = it!!) }
    private val locationsObserver: Observer<List<WeatherLocationModel>> =
            Observer { locationAdapter.updateData(newData = it!!) }

    override fun onAttach(context: Context?) {
        super.onAttach(context)

        viewModel = ViewModelProviders.of(this, factory)[ChooseLocationsViewModel::class.java]
    ...
    override fun onLocationReceived(location: WeatherLocationModel) {
        preferences.changeDefaultLocationStatus(isFromGps = false)

        preferences.changeGpsPermissionStatus(isOn = false)

        preferences.updateCustomCoordinates(
                location = location.name,
                latitude = location.latitude,
                longitude = location.longitude
        )

        activity?.let { goToMainScreen(currentScreen = it) }
    }
