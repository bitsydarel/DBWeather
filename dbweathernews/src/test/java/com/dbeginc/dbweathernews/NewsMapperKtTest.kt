...
package com.dbeginc.dbweathernews
...
class NewsMapperKtTest {
    ...
    fun `Testing convert from domain model to presentation model`() {
        ...
        val articleWith5Minute = Article("","", "", "", "", currentTime.minusMinutes(5).toInstant().toString(), "").toTestModel("Darel", currentTime)
    ...
    fun `Testing sorting of article by publish time`() {
        ...
        val articleWith2Minutes = Article("","", "", "", "", currentTime.minusMinutes(2).toInstant().toString(), "").toTestModel("Darel", currentTime)
        val articleWith5Hours = Article("","", "", "", "", currentTime.minusHours(5).toInstant().toString(), "").toTestModel("Darel", currentTime)
        val articleWith2Days = Article("","", "", "", "", currentTime.minusDays(2).toInstant().toString(), "").toTestModel("Darel", currentTime)
        val articleWith3days = Article("","","","", "", currentTime.minusDays(3).toInstant().toString(),"").toTestModel("Darel", currentTime)
        val articleWithNoTime = Article("","","","", "",null,"").toTestModel("Darel", currentTime)
    ...
    fun `Testing sorting of articles in collections`() {
        ...
        val articleWith2Minutes = Article("","", "", "", "", currentTime.minusMinutes(2).toInstant().toString(), "").toTestModel("Darel", currentTime)
        val articleWith5Minutes = Article("","", "", "", "", currentTime.minusHours(5).toInstant().toString(), "").toTestModel("Darel", currentTime)
        val articleWith5Hours = Article("","", "", "", "", currentTime.minusHours(5).toInstant().toString(), "").toTestModel("Darel", currentTime)
        val articleWith2Days = Article("","", "", "", "", currentTime.minusDays(2).toInstant().toString(), "").toTestModel("Darel", currentTime)
        val articleWith3days = Article("","","","", "", currentTime.minusDays(3).toInstant().toString(),"").toTestModel("Darel", currentTime)
        val articleWith5Days = Article("","","","", "", currentTime.minusDays(5).toInstant().toString(),"").toTestModel("Darel", currentTime)
        val articleWithNoTime = Article("","","","", "",null,"").toTestModel("Darel", currentTime)
